...
#' used as input to the neural network. If FALSE, the sequence of interarrival time (gap time 
#' between an event and the previous event) will be used as input to the neural network.
#' @param log logical. If TRUE, input of the neural net is the base-10 log of the original 
#' sequence of times plus 1 (i.e., log10(t+1)). If FALSE, the original sequence of times is used.
#' @param rnn_type the type of recurrent unit to be used for modeling
#'   the timestamp sequences. \code{"lstm"} for the long-short term memory unit. 
...
#'   for training. Available options are \code{"sgd"}, \code{"rmsprop"}, 
...
#' n <- 50
#' data(cc_data)
#' samples <- sample(1:length(cc_data$seqs$time_seqs), 50)
...
#' plot(time_seq2seq_res$train_loss, col="blue", type="l",ylim = range(c(time_seq2seq_res$train_loss,time_seq2seq_res$valid_loss)))
#' lines(time_seq2seq_res$valid_loss, col="red", type = 'l')
#' @export
tseq2feature_seq2seq <- function(tseqs, K, cumulative = FALSE, log = TRUE, rnn_type="lstm", n_epoch=50, method="last", step_size=0.0001, optimizer_name="rmsprop", 
                                 samples_train, samples_valid, samples_test=NULL, pca=TRUE, gpu=FALSE, verbose=TRUE, return_theta=TRUE) {
...
#'
#' \code{atseq2feature_seq2seq} extract features from actions and timestamps sequences by a 
#' sequence autoencoder.
#'
#' This function trains a sequence-to-sequence autoencoder using keras. The encoder
#' of the autoencoder consists of a recurrent neural network.
#' The decoder consists of another recurrent neural network followed by a fully connected layer
#' with softmax activation for actions and another fully connected layerwith ReLU activation 
...
#' @param K the number of features to be extracted.
#' @param weights a vector of 2 elements for the weight of the loss of action sequences (categorical_crossentropy)
#' and time sequences (mean squared error), respectively. The total loss is calculated as the weighted sum of the two 
#' losses.
#' @param cumulative logical. If TRUE, for the timestamp sequences, the sequence of cumulative time up to each event is  
#' used as input to the neural network. If FALSE, the sequence of interarrival times (gap time between an event and 
#' the previous event) will be used as input to the neural network.
#' @param log logical. If TRUE, for the timestamp sequences, input of the neural net is the base-10 log of the original 
#' sequence of times plus 1 (i.e., log10(t+1)). If FALSE, the original sequence of times is used.
#' @param rnn_type the type of recurrent unit to be used for modeling
#'   the action and time sequences. \code{"lstm"} for the long-short term memory unit. 
#'   \code{"gru"} for the gated recurrent unit.
#' @param n_epoch the number of training epochs for the autoencoder.
#' @param method the method for computing features from the output of an
#'   recurrent neural network in the encoder. Available options are 
...
#' plot(action_and_time_seq2seq_res$train_loss, col="blue", type="l",ylim = range(c(action_and_time_seq2seq_res$train_loss,action_and_time_seq2seq_res$valid_loss)))
#' lines(action_and_time_seq2seq_res$valid_loss, col="red", type = 'l')
#' @export
atseq2feature_seq2seq <- function(atseqs, K, weights = c(1, .5), cumulative = FALSE, log = TRUE, rnn_type="lstm", n_epoch=50, method="last", step_size=0.0001, optimizer_name="rmsprop", 
                                 samples_train, samples_valid, samples_test=NULL, pca=TRUE, gpu=FALSE, verbose=TRUE, return_theta=TRUE) {
